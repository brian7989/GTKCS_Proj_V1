...
class CheggHelper:
    ...
    def getQuesAns(self, q_url):
        try:
            ...
            print(f"Sleeping for {sleepTime} to avoid web scrapping detection from Chegg")
            sleep(sleepTime)
            ...
            if 'questions-and-answers' in q_url:
                ...
                ans = ''
                filename = './answers/answer_' + ''.join(
                    random.choice(string.ascii_lowercase + string.digits) for _ in range(10)) + '.html'
                ...
                if js1['httpCode'] == 200:
                    if len(js1['result']['solutions']) > 0:
                        ...
                        for indd, x in enumerate(js1['result']['solutions'][0]['steps']):
                            datac += '<br><H1>Step - ' + str(indd + 1) + '<br>' + requests.get(x['link'],
                                                                                               verify=False).text

                        # with open(filename, 'w', encoding='utf-8') as fp:
                        #     fp.write(str(datac))
                        # return True, filename, 'Success'
                        return True, datac, "Success"
                    else:
